...
public class UT_BN254Curve
{
    ...
    // A point with y == 0 is 2-torsion: its tangent is vertical, so [2]P is the point at
    // infinity. Doubling must return infinity rather than dividing by 2y == 0 and throwing.
    // This keeps the law total, which matters because the G2 subgroup check multiplies an
    // attacker-supplied on-curve point by the curve order; a crafted 2-torsion input must be
    // rejected gracefully (TryDeserializeG2 returning false) instead of faulting.
    [TestMethod]
    public void EcPoint_DoublingPointWithZeroYIsInfinity()
    {
        var twoTorsion = EcPoint<Fp>.Create(Fp.FromInteger(7), Fp.Zero);
        Assert.IsTrue(twoTorsion.Double().IsInfinity);
    }

    [TestMethod]
    public void EcPoint_AddingPointWithZeroYToItselfIsInfinity()
    {
        var twoTorsion = EcPoint<Fp>.Create(Fp.FromInteger(7), Fp.Zero);
        Assert.IsTrue(twoTorsion.Add(twoTorsion).IsInfinity);
    }
