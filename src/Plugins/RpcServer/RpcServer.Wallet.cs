...
partial class RpcServer
{
    ...
    protected internal virtual JToken SendFrom(UInt160 assetId, Address from, Address to, string amount, Address[]? signers = null)
    {
        ...
        using var snapshot = system.GetSnapshotCache();
        var descriptor = new AssetDescriptor(snapshot, assetId);
    ...
    protected internal virtual JToken CancelTransaction(UInt256 txid, Address[] signers, string? extraFee = null)
    {
        ...
        {
            var descriptor = new AssetDescriptor(snapshot, NativeContract.Governance.GasTokenId);
    ...
    /// </returns>
    /// <exception cref="RpcException">
    /// Thrown when the blockchain rejects the relay (e.g. <see cref="VerifyResult.Expired"/>,
    /// <see cref="VerifyResult.PolicyFail"/>). This keeps wallet
    ...
    private JObject SignAndRelay(DataCache snapshot, Transaction tx)
    {
        ...
        if (NativeContract.TokenManagement.BalanceOf(snapshot, NativeContract.Governance.GasTokenId, tx.Sender) < tx.SystemFee + tx.NetworkFee)
