...
namespace Neo.Cryptography.BN254Curve
{
    ...
    internal readonly struct EcPoint<T> where T : IFieldElement<T>
    {
        ...
        public EcPoint<T> Double()
        {
            ...
            // A point with y == 0 is 2-torsion: the tangent is vertical, so [2]P is the point at
            // infinity. Returning it here keeps doubling total and avoids dividing by 2y == 0. This
            // matters for the G2 subgroup check, which multiplies an attacker-supplied on-curve
            // point by the curve order; a crafted 2-torsion input is then rejected gracefully
            // (TryDeserializeG2 returns false) instead of faulting out of a bool-returning method.
            if (Y.IsZero)
                return Infinity;
